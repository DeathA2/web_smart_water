...
//             children: [
//               InkWell(
//                 onTap: () => appController.changeRoute(RouteModel(route: appController.currentRoute.value.group, label: appController.currentRoute.value.group == '/bao_cao'?'Báo cáo':'Chốt số', group: appController.currentRoute.value.group, children: [], screen: appController.currentRoute.value.group == '/bao_cao'?const ListReportScreen():const ListReportFinalizationScreen())),
//                 child: Text(
//                   appController.currentRoute.value.group == '/bao_cao'?'Báo cáo':'Chốt số',
//                   style:ThemeConfig.defaultStyle.copyWith(color: ThemeConfig.hoverTextColor,fontWeight: FontWeight.bold),
//                 ),
//               ),
...
//           (controller.template['buttons'] as List).length>1?PopupMenuButton<dynamic>(
//             padding: EdgeInsets.zero,
//             icon: Icon(Icons.arrow_drop_down,color: ThemeConfig.whiteColor,size: 20,),
//             itemBuilder: (context) => (controller.template['buttons'] as List).getRange(1,(controller.template['buttons'] as List).length).map((e) => _buildDropdownActionItem(e,context)).toList(),
...
//       child: Container(
//         width: 150,
//         padding: EdgeInsets.symmetric(vertical: ThemeConfig.defaultPadding/2,horizontal: ThemeConfig.defaultPadding/2),
...
//           return AlertDialog(
//             scrollable: true,
//             content: MyCreateView(model: controller.typeModel.getEmptyModel(),isNew: true,),
//           );
//         }).then((value){
//       if(value != null){
...
//           controller.selectb2e?'Từ ngày: ${controller.beginController.text}':'Ngày: ${controller.beginController.text}');
//       //
...
//             'Đến ngày: ${controller.endController.text}');
//       }
...
//     }
//     bool checkColDelete = (controller.typeModel.getListViewTemplate()['fields'] as List).where((element) => element['field'] == 'action_button').isNotEmpty;
//     if(checkColDelete) {
//       sheet.deleteColumn(
//           (controller.typeModel.getListViewTemplate()['fields'] as List).length);
//     }
//     final List<int> bytes = workbook.saveAsStream();
//     String path = await FileSaver.instance.saveFile('${controller.titleName}',Uint8List.fromList(bytes),'xlsx',mimeType: MimeType.MICROSOFTEXCEL);
//     print(path);
//     showDialog(
//       context: context,
//       builder: (context) {
