...
//     this.sumCol,required this.indexColumn,
...
//     _dataSource = DataSource(dataAll: dataList, column: column,mapColumnName: widget.mapColumnName,callback: (image)=> widget.callback(image));
...
//       sheet
//           .getRangeByName('B2:C2')
//           .cellStyle
//           .hAlign = HAlignType.left;
//       sheet.getRangeByName('B2').setText(
//           widget.selectb2e?'Từ ngày: ${beginController.text}':'Ngày: ${beginController.text}');
//       //
//       if(widget.selectb2e) {
//         sheet.getRangeByName('D2:E2').merge();
//         sheet
//             .getRangeByName('D2:E2')
//             .cellStyle
//             .hAlign = HAlignType.left;
//         sheet.getRangeByName('D2').setText(
//             'Đến ngày: ${endController.text}');
//       }
//     }
...
//         }
//         sheet.getRangeByIndex(rowNext, colIndex+1).setText(appController.getMoneyFormat(_sumDataByKey(UtilFunction.getColumn(col)).toString()));
...
//         sheet
//             .getRangeByName('B$rowNext:C$rowNext')
//             .cellStyle
//             .bold = true;
//         sheet.getRangeByName('B$rowNext').setText(
//             '${widget.titleCount}:');
//         //
//         sheet
//             .getRangeByName('D$rowNext')
//             .cellStyle
//             .bold = true;
...
//     String path = await FileSaver.instance.saveFile(widget.exportTitle.toUpperCase(),Uint8List.fromList(bytes),'xlsx',mimeType: MimeType.MICROSOFTEXCEL);
//     print(path);
//     showDialog(
//       context: context,
//       builder: (context) {
...
//       dataList = widget.data.isEmpty ? [] : widget.data;
//       _dataSource =
//           DataSource(dataAll: appController.reportData, column: column,mapColumnName: widget.mapColumnName,callback: (image)=> widget.callback(image));
//       for (String col in column) {
//         if(UtilFunction.isFilter(col)) {
//           allFilter[col] = _getAllValueByKey(col);
//         }
...
//       _dataSource = DataSource(dataAll: currentList, column: column,mapColumnName: widget.mapColumnName,callback: (image)=> widget.callback(image));
//     });
//   }
//   void _resetFilter(){
//     setState(() {
...
//     return widget.changeDate?Container(
...
//           widget.selectb2e?ResponsiveGridCol(
//               lg: 3,
//               child:MyInputDate(
//                 padding: EdgeInsets.zero,
...
//           return true;
//         },
//         footer: _dataSource._data.isEmpty?Container(
//             color: Colors.grey[400],
//             child: const Center(
//                 child: Text(
//                   'No data',
//                   style: TextStyle(fontWeight: FontWeight.bold),
//                 ))):(sumCol.isEmpty && !widget.countRecord)?null:_buildSum(),
...
//   }
//   Widget _buildSum(){
//     return (widget.countRecord && sumCol.isEmpty)?Row(
//       children: [
//         SizedBox(width: (MediaQuery.of(context).size.width-103)/(column.isEmpty?1:column.length)*2),
//         Text('${widget.titleCount}: ',style: TextStyle(fontSize: ThemeConfig.smallSize,fontWeight: FontWeight.bold)),
//         SizedBox(width: (MediaQuery.of(context).size.width-103)/(column.isEmpty?1:column.length)/2),
...
//               child:Text(appController.getMoneyFormat(_sumDataByKey(UtilFunction.getColumn(e)).toString()),style: TextStyle(fontSize: ThemeConfig.smallSize,fontWeight: FontWeight.bold),),
...
//                 ),
//               ),
//               UtilFunction.isFilter(e)?InkWell(
//                 onTap:() => _showMyDialog(e),
//                 child: Icon((filter[e] as List).isNotEmpty?Icons.filter_list_alt:Icons.filter_alt_outlined,size: ThemeConfig.defaultSize,),
//               ):Container()
//             ],
//           ),
//         )
...
//           });
//         },
//         pageCount: (dataList.length / _rowsPerPage).ceil().toDouble()==0?1:(dataList.length / _rowsPerPage).ceil().toDouble(),
...
//     return appController.isLoading.value?const LoadingScreen():Column(
//       crossAxisAlignment: CrossAxisAlignment.start,
//       children: [
...
//         setState(() {
//           curentFilter = allFilter[col]?.where((element) => (element.toString().toLowerCase()).contains(search.text.toLowerCase())).toList();
//         });
//       }else{
//         setState(() {
...
//                           if(value.isNotEmpty){
//                             setState(() {
//                               curentFilter = allFilter[col]?.where((element) => (element.toString().toLowerCase()).contains(value.toLowerCase())).toList();
//                             });
//                           }else{
//                             setState(() {
//                               curentFilter = allFilter[col];
...
//                         },
//                         child: ListTile(
//                           leading: Checkbox(value: (filter[col] as List).length == curentFilter?.length, onChanged: (bool? value) {
//                             setState(() {
//                               selectAll = value!;
//                               if(value){
//                                 filter[col] = curentFilter;
...
//                                   },
//                                   child: ListTile(
//                                     leading: Checkbox(value: (filter[col] as List).contains(e), onChanged: (bool? value) {
//                                       setState(() {
//                                         if(value!){
//                                           (filter[col] as List).add(e);
//                                         }
...
//   DataSource({required this.callback,required List<Map<String, dynamic>> dataAll,required this.column,required this.mapColumnName}) {
//     _data = dataAll;
...
//           Color getColor() {
//             if (searchController.text.isNotEmpty &&dataGridCell.value.toString().toLowerCase().contains(searchController.text.toLowerCase())) {
...
//             child: dataGridCell.columnName != 'Image'?SelectableText(dataGridCell.value.toString(),maxLines: 10,style:  TextStyle(
//                 fontSize: ThemeConfig.smallSize,
//                 color: ThemeConfig.textColor
//             ),):InkWell(
...
//       super.sort();
//     }else if(!UtilFunction.columnNotSort.contains(sortedColumns.first.name)) {
//       super.sort();
//     }else{
//       return;
//     }
...
//     appController.dataGridController.scrollToVerticalOffset(0);
//     _dataGridRow = _paginatedData.map<DataGridRow>((e) => DataGridRow(
//         cells: column.map((_column) {
//           return DataGridCell(columnName:UtilFunction.getColName(_column,mapColumnName), value: _getValueDisplay(_column,e[UtilFunction.getColumn(_column)]));
//         }).toList()
//     )).toList(growable: false);
//   }
//
//   List<DataGridRow> buildAllDataGridRows() {
//     return _data.map<DataGridRow>((e) => DataGridRow(
//         cells: column.map((_column) {
//           return DataGridCell(columnName: UtilFunction.getColName(_column,mapColumnName),value: e[_column].toString());
//         }).toList()
//     )).toList(growable: false);
//   }
...
//     'loaihang':'Loại hàng'
//   };
//   static const columnNotFilter =['id','code','index','des','soluong','khoiluong','number','saiso','trongluong','khoiluong_pref','time','image1'];
