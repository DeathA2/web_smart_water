...
import '../ui/item/template_web/list_view/search.dart';
import '../ui/widget/custom_table_source.dart';
import '../ui/widget/loading_screen.dart';
import '../ui/widget/my_input_date.dart';


class TemplateReport extends StatefulWidget{
  const TemplateReport({Key? key,
    required this.isEdit, required this.controller}) : super(key: key);
  final bool isEdit;
...
class _TemplateReportState extends State<TemplateReport> {
  ...
  void initState() {
    _loadData();
    column.addAll(
        (widget.controller.template['fields'] as List).map((e) => e['field']).toList().cast<String>()
    );
    sumCol = widget.controller.typeModel.getListViewTemplate()['sum_col']??[];
    sumRow.add(GridSummaryColumn(
        name: 'Count',
        columnName: column[0],
        summaryType: GridSummaryType.count));
    sumRow.addAll(sumCol.map((col) => GridSummaryColumn(
        name: 'Sum',
        columnName: col,
        summaryType: GridSummaryType.sum) ).toList());
    _setData();
    super.initState();
  }
    ...
    return Container(
      ...
      child: ResponsiveGridRow(
        children: [
          ...
          widget.controller.selectb2e?ResponsiveGridCol(
              lg: 3,
              child:Padding(
                padding: EdgeInsets.symmetric(horizontal: ThemeConfig.defaultPadding/2),
                child: MyInputDate(
  ...
  Widget _buildTable(context) {
    return  Container(
      ...
      child: SfDataGrid(
        ...
        rowHeight: widget.controller.rowHeight,
        ...
        label: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              widget.controller.typeModel.isFilter(field['field'],true)?InkWell(
